...
const questions=[
  {lesson:'Matematik', topic:'Yüzdeler', q:'Bir sayının %25’i 18 ise bu sayı kaçtır?', o:['36','54','72','90'], a:2, exp:'18 sayısı sayının dörtte biridir. 18 x 4 = 72.'},
  {lesson:'Türkçe', topic:'Cümlede anlam', q:'“Kitap okumayı çok severim.” cümlesinde yüklem hangisidir?', o:['Kitap','Okumayı','Çok','Severim'], a:3, exp:'Cümlede işi bildiren kelime “severim”dir.'},
  {lesson:'Fen Bilimleri', topic:'Fotosentez', q:'Fotosentez için aşağıdakilerden hangisi doğrudan gereklidir?', o:['Işık','Tuz','Kum','Ses'], a:0, exp:'Bitkiler fotosentez için ışığa ihtiyaç duyar.'},
  {lesson:'İngilizce', topic:'Vocabulary', q:'“Book” kelimesinin Türkçe karşılığı hangisidir?', o:['Kalem','Kitap','Masa','Okul'], a:1, exp:'Book = Kitap.'}
];
const app = document.getElementById('app');
function saveRole(r){ state.role=r; localStorage.setItem('sac_role', r); renderLogin(); }
function renderLogin(){
  const r = users[state.role];
  app.innerHTML = `<div class="loginPage">
    <section class="loginHero">
      <div class="brand big"><span class="logoStar">★</span><div>STAR ACADEMY<br><small>CYPRUS</small></div></div>
      <div class="heroCopy"><span class="eyebrow">KKTC Fen Lisesi & BEAL Hazırlık Platformu</span><h1>Akıllı soru havuzu, deneme sınavı ve veli takip sistemi.</h1><p>Çıkmış sorular, hızlı testler, mini konu anlatımları ve kişiye özel başarı analizi tek platformda.</p></div>
      <div class="heroStats"><div><b>${QUESTION_BANK.length}</b><span>PDF arşiv</span></div><div><b>5</b><span>Ana ders</span></div><div><b>3</b><span>Panel tipi</span></div></div>
    </section>
    <section class="loginSide"><div class="loginCard">
      <div class="brand center"><span class="logoStar">★</span><div>STAR ACADEMY<br><small>CYPRUS</small></div></div>
      <h2>Giriş Yap</h2><p class="muted">Hesap türünü seçerek sisteme giriş yap.</p>
      <div class="roleGrid">${Object.keys(users).map(k=>`<button class="role ${state.role===k?'active':''}" onclick="saveRole('${k}')">${roleLabel(k)}</button>`).join('')}</div>
      <label>Kullanıcı adı</label><input id="username" value="${r.username}">
      <label>Şifre</label><input id="password" type="password" value="${r.password}">
...
function studentHome(){return `<section class="metrics topMetrics">${metric('📚','Toplam Test','1.248','Çözülen test sayısı')}${metric('✓','Doğru','892','%71.5 Başarı','green')}${metric('✕','Yanlış','356','%28.5 Hata','red')}${metric('⏱','Çalışma Süresi','78sa 40dk','Toplam süre','orange')}${metric('📊','Genel Net','+536.75','Net ortalaman','purple')}</section><section class="studentGrid"><div class="heroPanel"><h2>Fen Lisesi & BEAL<br>Hayaline ulaşmanın<br>en doğru yolu!</h2><p>Planlı çalış, düzenli tekrar yap ve hedefini gerçekleştir!</p><button onclick="state.screen='solve';render()">Hemen Çalışmaya Başla →</button></div><div class="panel"><h2>Günlük Görevlerin</h2>${tasks.map(t=>`<div class="goal"><b>${t[0]}</b><span>${t[1]}</span><div class="progress"><i style="width:${t[3]}%"></i></div></div>`).join('')}</div></section><section class="three">${subjectCards()}<div class="panel quick"><h2>Hızlı İşlemler</h2><button onclick="state.screen='mini';render()">⚡<span>Mini Test</span></button><button onclick="state.screen='exams';render()">📄<span>Deneme Sınavı</span></button><button onclick="state.screen='wrong';render()">✕<span>Yanlışlarım</span></button><button onclick="state.screen='bank';render()">📁<span>Çıkmış Sorular</span></button></div></section>${recent()}<section class="three miniRow"><div></div>${teacherNote()}</section>`}
...
function bankPage(){
  ...
  return `<div class="panel"><div class="panelHead"><div><h2>Soru Havuzu ve Çıkmış Sınav Arşivi</h2><p>Yüklediğin arşiv sisteme bağlandı. PDF soru kitapçıkları ve cevap anahtarları yıllara göre listelenir.</p></div><a href="assets/lise-deneme-programi-soru-havuzu.rar" download>Orijinal arşivi indir</a></div><div class="filters"><input placeholder="Soru kitapçığı, ders veya yıl ara..." value="${state.query}" oninput="state.query=this.value;render()"><select onchange="state.filter=this.value;render()">${years.map(y=>`<option ${state.filter===y?'selected':''}>${y}</option>`).join('')}</select></div><div class="bankGrid">${list.map(x=>`<article class="bankCard"><span>${x.year}</span><h3>${x.title}</h3><p>${x.session} • ${x.type}</p><div>${x.lessons.map(l=>`<em>${l}</em>`).join('')}</div><small>${(x.size/1024/1024).toFixed(2)} MB</small></article>`).join('')}</div></div>`
}
function solvePage(){let q=questions[state.q];return `<div class="solveWrap"><div class="panel questionPanel"><div class="panelHead"><span>${q.lesson} • ${q.topic}</span><b>Soru ${state.q+1}/${questions.length}</b></div><h2>${q.q}</h2>${q.o.map((o,i)=>`<button class="option ${state.selected===i?(i===q.a?'ok':'bad'):''}" onclick="state.selected=${i};render()"><b>${String.fromCharCode(65+i)}</b>${o}</button>`).join('')}${state.selected!==null?`<div class="answer"><b>Açıklama:</b> ${q.exp}</div><button class="primary" onclick="state.q=(state.q+1)%questions.length;state.selected=null;render()">Sonraki Soru</button>`:''}</div><div class="panel"><h2>Akıllı Çalışma</h2><p>Yanlış yaptığın konular otomatik olarak “Konu Eksikleri” listesine eklenir.</p>${weakPanel()}</div></div>`}
